    ...
    class CudaGraphTreeTests(TestCase):
        ...
        def test_static_inputs_address_mutation_log(self):
            ...
            with self.assertRaisesRegex(
                ...
                "input name: primals_2. data pointer changed from .* to .*\n"
                "input name: primals_3. data pointer changed from .* to .*\n"
                "related graph:(.*\n)*"
                "related stack traces:.*, in forward.* return self.static_tensor \+ x \+ self.goo\(x\).*\n"
            ):
                self.curr_node().run([foo.goo.linear.weight, foo.goo.linear.bias, foo.static_tensor, inp])
