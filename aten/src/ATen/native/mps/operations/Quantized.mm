...
kernel void int4pack_mm(
    ...
    const uint nb = group_index.x % (N/32); // 0..N/32-1
    const uint m = group_index.x / (N/32);  // 0..M-1
...
Tensor _weight_int4pack_mm_mps(const Tensor& A, const Tensor& B, int64_t qGroupSize, const Tensor& qScaleAndZeros) {
  ...
  dispatch_sync_with_rethrow(mpsStream->queue(), ^() {
    @autoreleasepool {
      ...
      [computeEncoder dispatchThreads:MTLSizeMake(M * N / 2, 4, 1) threadsPerThreadgroup:MTLSizeMake(16, 4, 1)];
#if _CAPTURE_KERNEL
